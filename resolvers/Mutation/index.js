...
const Mutation = {
	...
	async deleteProjectPhotos(_, { id }) {
		const deletedProjectPhoto = await db('project_photos').del().where('id', id);
		try {
	...
	async search(_, { text }) {
		...
		try {
			...
			const users = await db('users')
				...
				.orWhereRaw(`LOWER(CONCAT("firstName", "lastName")) LIKE ?`, [ `%${userText}%` ]);
	...
	async deleteProjectInvite(_, { id }) {
		const deletedProjectInvite = await db('user_research').del().where('id', id);
		try {
