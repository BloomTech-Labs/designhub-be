  ...
  try {
    ...
    if (!(await userMatches(req.user, project[0].userId))) {
      return res
        .status(401)
        .json({
          message: 'Unauthorized: You may not add photos to this project.',
        });
    }
...
exports.deleteUserResearchById = async (req, res) => {
  ...
  try {
    ...
    if (research.length === 0) {
      res
        .status(404)
        .json({ message: 'User research with that ID could not be found!' });
    } else {
      ...
      if (await userMatches(req.user, project[0].userId)) {
        ...
        res
          .status(200)
          .json({ message: 'Successfully deleted user research.' });
      } else {
        res.status(401).json({
          message:
