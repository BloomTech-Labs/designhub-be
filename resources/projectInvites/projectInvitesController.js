...
exports.getInviteById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: 'An unknown error occurred while getting this invite' });
  }
};
  ...
  try {
    ...
    if (
      ...
      if (invite[0].userId === null) {
        ...
        return res
          .status(200)
          .json({ message: 'This invite has been deleted' });
      } else {
        const notification = await db('invite')
          .where('projectId', project[0].id)
          .andWhere('message', invite[0].id + ' ' + invite[0].email);
